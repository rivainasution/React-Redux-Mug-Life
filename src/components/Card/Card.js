...
const ProductCard = ({ data, originalPrice, salePrice, rating, discount, sale }) => {
  ...
  const ratingHandle = (rating) => {
    ...
    if (hasHalfStar) {
      stars.push(<FontAwesomeIcon key={stars.length} icon={halfStar} className='text-warning' />);
    }
  
    const emptyStars = 5 - stars.length;
    for (let i = 0; i < emptyStars; i++) {
      stars.push(<FontAwesomeIcon key={stars.length + i} icon={solidStar} className='text-secondary' />);
    }
